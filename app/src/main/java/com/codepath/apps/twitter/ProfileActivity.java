...
public class ProfileActivity extends AppCompatActivity {
	...
	private void fetchTimeline() {
		client.getScrolledProfile(user.uid, minSeenID, new JsonHttpResponseHandler() {
			@Override
			...
			public void onFailure(int statusCode, Header[] headers, Throwable throwable, JSONObject errorResponse) {
				if(statusCode == 429) {
					Toast.makeText(ProfileActivity.this, "Twitter Rate Limit Exceeded", Toast.LENGTH_LONG).show();
				}
	...
	private void populateTimeline() {
		client.getProfile(user.uid, new JsonHttpResponseHandler(){
			@Override
			public void onSuccess(int statusCode, Header[] headers, JSONArray response) {
				List<Tweet> list = new ArrayList<Tweet>();
				for(int i = 0; i < response.length(); i++) {
			...
			public void onFailure(int statusCode, Header[] headers, Throwable throwable, JSONObject errorResponse) {
				if(statusCode == 429) {
					Toast.makeText(ProfileActivity.this, "Twitter Rate Limit Exceeded", Toast.LENGTH_SHORT).show();
				}
				Log.d("TwitterClient", errorResponse.toString());
				throwable.printStackTrace();
			}
